package com.bamboy.bimage.view.scrollpicker;
...
public abstract class ScrollPickerView<T> extends View {
     ...
     * 是否手指抬起并正在惯性滑动
     */
    private boolean mIsFingerUpAndFling;
    /**
     * 是否正在滑向中间
     */
    ...
    public void computeScroll() {
        if (mScroller.computeScrollOffset()) { // 正在滚动
            ...
            invalidate();
            if (mIsFingerUpAndFling == false) {
                mIsFingerUpAndFling = true;
                if (mListener != null)
                    mListener.onFling(mIsFingerUpAndFling);
            }
            if (mRealTime) {
                notifySelected();
            }
    ...
    public interface OnSelectedListener {
         ...
         * @param flingStart
         */
        void onFling(boolean flingStart);
    }
